...
public class QuestNode_GetSeaSiteTile : QuestNode_GetSiteTile
{
  ...
  private bool TryFindTile(Slate slate, out PlanetTile tile)
  {
    ...
    var nearTile =
      (slate.Get<Map>("map") ?? (value ? Find.RandomPlayerHomeMap : Find.RandomSurfacePlayerHomeMap))?.Tile ??
      PlanetTile.Invalid;
    ...
    return TileFinderSea.TryFindNewSiteTile(out tile, nearTile, var.min, var.max, allowCaravans.GetValue(slate),
      allowedLandmarks.GetValue(slate), 0.5f, canSelectComboLandmarks.GetValue(slate), tileFinderMode,
