...
public class GenStep_HumanTraces : GenStep
{
  ...
  public override void Generate(Map map, GenStepParams parms)
  {
    ...
    if (CellFinder.TryFindRandomCellNear(cell, map, 4, c =>
            !c.Fogged(map) && GenSpawn.CanSpawnAt(MVO_DefOf.MVO_Filth_TallyMarks, c, map, Rot4.North) &&
            c.GetRoof(map) is not null,
          out var cell5))
    {
    ...
    if (ModsConfig.BiotechActive)
    {
      ...
      {
        if (CellFinder.TryFindRandomCellNear(cell, map, 4, c =>
                !c.Fogged(map) && GenSpawn.CanSpawnAt(ThingDefOf.Filth_Floordrawing, c, map, Rot4.North) &&
                c.GetRoof(map) is not null,
              out var cell6))
        {
