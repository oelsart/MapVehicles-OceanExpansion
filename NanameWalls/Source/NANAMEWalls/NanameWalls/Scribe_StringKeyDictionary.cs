...
namespace NanameWalls
{
  ...
  public static class Scribe_StringKeyDictionary
  {
    ...
    public static void Look<V>(ref Dictionary<string, V> dict, string label,
      LookMode valueLookMode = LookMode.Undefined)
    {
      if (Scribe.mode == LoadSaveMode.LoadingVars)
      {
        ...
        {
          Log.Error(
            "You need to provide working lists for the keys and values in order to be able to load such dictionary. label=" +
            label);
        }
      }
    ...
    public static void Look<V>(ref Dictionary<string, V> dict, string label, LookMode valueLookMode,
      ref List<string> keysWorkingList, ref List<V> valuesWorkingList, bool logNullErrors = true,
      bool saveDestroyedValues = false)
    {
    ...
    private static void Look<T>(ref List<string> keyList, ref List<T> valueList, bool saveDestroyedThings, string label,
              ...
              for (var i = 0; i < num; i++)
              {
                ...
                if (typeof(T).IsGenericType && typeof(T).GetGenericTypeDefinition() == typeof(List<>))
                {
                  throw new InvalidOperationException(
                    "This case should be impossible; it should be calling the list-of-lists overload.");
                }
                ...
                switch (lookMode)
                {
                  ...
                  case LookMode.Reference:
                  {
                    ...
                    {
                      throw new InvalidOperationException("Cannot save reference to " +
                                                          valueList[i]?.GetType()?.ToStringSafe() +
                                                          " item if it is not ILoadReferenceable");
                    }
                ...
                case LookMode.LocalTargetInfo:
                {
                  ...
                  var num3 = 0;
                  {
                    foreach (XmlNode childNode5 in curXmlParent.ChildNodes)
                    {
                      ...
                      var item4 = (T)(object)ScribeExtractor.LocalTargetInfoFromNode(childNode5, num3.ToString(),
                ...
                case LookMode.TargetInfo:
                {
                  ...
                  var num2 = 0;
                  {
                    foreach (XmlNode childNode6 in curXmlParent.ChildNodes)
                    {
                      ...
                      var item3 = (T)(object)ScribeExtractor.TargetInfoFromNode(childNode6, num2.ToString(),
                        TargetInfo.Invalid);
                ...
                case LookMode.GlobalTargetInfo:
                {
                  ...
                  var num = 0;
                  {
                    foreach (XmlNode childNode7 in curXmlParent.ChildNodes)
                    {
                      ...
                      var item2 = (T)(object)ScribeExtractor.GlobalTargetInfoFromNode(childNode7, num.ToString(),
            ...
            switch (lookMode)
            {
              ...
              case LookMode.LocalTargetInfo:
                if (valueList != null)
                {
                  ...
                  {
                    valueList[j] =
                      (T)(object)ScribeExtractor.ResolveLocalTargetInfo((LocalTargetInfo)(object)valueList[j],
                        j.ToString());
                  }
                }
              ...
              case LookMode.TargetInfo:
                if (valueList != null)
                {
                  ...
                  {
                    valueList[k] =
                      (T)(object)ScribeExtractor.ResolveTargetInfo((TargetInfo)(object)valueList[k], k.ToString());
                  }
                }
              ...
              case LookMode.GlobalTargetInfo:
                if (valueList != null)
                {
                  ...
                  {
                    valueList[i] =
                      (T)(object)ScribeExtractor.ResolveGlobalTargetInfo((GlobalTargetInfo)(object)valueList[i],
                        i.ToString());
                  }
                }
    ...
    private static void BuildDictionary<V>(Dictionary<string, V> dict, List<string> keysWorkingList,
      List<V> valuesWorkingList, string label, bool logNullErrors)
    {
      if (dict == null)
      {
        return;
      ...
      for (var i = 0; i < num; i++)
      {
        if (keysWorkingList[i] == null)
        {
          if (logNullErrors)
          {
            Log.Error("Null key while loading dictionary of " + typeof(string).ToString() + " and " +
                      typeof(V)?.ToString() + ". label=" + label);
          }
