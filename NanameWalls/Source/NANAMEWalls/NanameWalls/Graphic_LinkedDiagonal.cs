...
public class Graphic_LinkedDiagonal(Graphic subGraphic) : Graphic_LinkedCornerFiller(subGraphic)
{
  ...
  static Graphic_LinkedDiagonal()
  {
    ...
    {
      Bottom = new Material(
        MaterialPool.MatFrom("Terrain/Surfaces/Substructure/SubstructureProps_Bottom", ShaderDatabase.CutoutFlying))
        {
          renderQueue = 2899
        };
    });
  ...
  private static readonly Vector3[] SubstructureBottomUVs =
  [
    new(0f, 0f),
    new(0f, 1f),
    new(1f, 1f),
  ...
  private static readonly Vector3[] SubstructureBottomVerts =
  [
    new(0.5f, 0f, -1.5f),
    new(0.5f, 0f, -0.5f),
    new(1.5f, 0f, 0.5f),
  ...
  [Obsolete("Changed to a constant.")] public static LinkDrawerType LinkerTypeStatic => (LinkDrawerType)LinkerNumber;
  ...
  protected override Material LinkedDrawMatFrom(Thing parent, IntVec3 cell)
  {
    ...
    var east2 = !settings.allowVShaped && ShouldLinkWith(pos + IntVec3.East * 2, parent, false);
    var west2 = !settings.allowVShaped && ShouldLinkWith(pos + IntVec3.West * 2, parent, false);
    var north2 = !settings.allowVShaped && ShouldLinkWith(pos + IntVec3.North * 2, parent, false);
    var south2 = !settings.allowVShaped && ShouldLinkWith(pos + IntVec3.South * 2, parent, false);
    if ((linkDirections & LinkDirections.Right) <= LinkDirections.None && !east2 && southEast ^ northEast)
    {
      if (southEast
            ? ClearFor(settings, Rot4.East, Rot4.North, parent)
            : ClearFor(settings, Rot4.East, Rot4.South, parent))
      {
        linkDirections |= LinkDirections.Right;
      }
    }
    ...
    {
      if (southWest
            ? ClearFor(settings, Rot4.West, Rot4.North, parent)
            : ClearFor(settings, Rot4.West, Rot4.South, parent))
      {
        linkDirections |= LinkDirections.Left;
      }
    }
    ...
    {
      if (!south2 && (southEast
            ? ClearFor(settings, Rot4.South, Rot4.West, parent)
            : ClearFor(settings, Rot4.South, Rot4.East, parent)))
      {
    ...
    {
      if (!north2 && (northEast
            ? ClearFor(settings, Rot4.North, Rot4.West, parent)
            : ClearFor(settings, Rot4.North, Rot4.East, parent)))
      {
        linkDirections |= LinkDirections.Up;
      }
    }
  ...
  public override void Print(SectionLayer layer, Thing thing, float extraRotation)
  {
    ...
    var north2 = !settings.allowVShaped && ShouldLinkWith(pos + IntVec3.North * 2, thing, false);
    var east = ShouldLinkWith(pos + IntVec3.East, thing);
    var east2 = !settings.allowVShaped && ShouldLinkWith(pos + IntVec3.East * 2, thing, false);
    ...
    if (southEast && ClearFor(settings, Rot4.South, Rot4.West, thing) &&
      ...
      PrintConditional(layer, thing, settings, center + new Vector3(1f, 0f, -1f), extraRotation, true,
        Condition.LinkedOther);
    if (southWest && ClearFor(settings, Rot4.South, Rot4.East, thing) &&
      ...
      PrintConditional(layer, thing, settings, center + new Vector3(-1f, 0f, -1f), extraRotation, false,
        Condition.LinkedOther);
    ...
    // Substructure props
    var terrGrid = thing.Map.terrainGrid;
    if (NanameWalls.Mod.Settings.renderSubstructure &&
        ModsConfig.OdysseyActive &&
        terrGrid.FoundationAt(pos) is { IsSubstructure: true })
    {
      if ((flag & Diagonals.SouthEast) > Diagonals.None &&
          terrGrid.FoundationAt(pos + IntVec3.NorthEast) is { IsSubstructure: true } &&
          terrGrid.FoundationAt(pos + IntVec3.East) is not { IsSubstructure: true })
      {
        PrintSubstructureBottom(layer, center, false, extraRotation);
      }

      if ((flag & Diagonals.SouthWest) > Diagonals.None &&
          terrGrid.FoundationAt(pos + IntVec3.NorthWest) is { IsSubstructure: true } &&
          terrGrid.FoundationAt(pos + IntVec3.West) is not { IsSubstructure: true })
      {
        PrintSubstructureBottom(layer, center, true, extraRotation);
      }
    }
    ...
    if (north)
    {
      if ((flag & Diagonals.SouthEast) > Diagonals.None)
      {
        Printer_Plane.PrintPlane(layer, center + new Vector3(0.4f, 0f, 0.75f).RotatedBy(extraRotation),
          Vector2.one * 0.75f, mat, extraRotation, false, CornerFillerUVs);
      }
    ...
    {
      Printer_Plane.PrintPlane(layer, center + new Vector3(0.55f, 0f, 0.5f).RotatedBy(extraRotation),
        Vector2.one * 0.75f, mat, extraRotation, false, CornerFillerUVs);
      Printer_Plane.PrintPlane(layer, center + new Vector3(0.75f, 0f, 0.75f).RotatedBy(extraRotation),
        Vector2.one * 0.5f, mat, extraRotation, false, CornerFillerUVs);
    }
  ...
  private void PrintConditional(SectionLayer layer, Thing thing, MeshSettings settings, Vector3 center,
    float extraRotation, bool flipped, Condition condition)
  {
  ...
  private void PrintDiagonal(SectionLayer layer, Thing thing, MeshSettings settings, Vector3 center,
    float extraRotation, Diagonals direction, bool finishPrint = false)
  {
    ...
    var vertsDirection = north ? finishPrint ? Condition.SouthFinish : Condition.North :
      finishPrint ? Condition.NorthFinish : Condition.South;
    foreach (var vertsItem in settings.settingItems.Values)
    {
  ...
  private static void PrintSubstructureBottom(SectionLayer layer, Vector3 center, bool flipped, float extraRotation)
  {
    var index = flipped ? TrisIndexFlipped : TrisIndex;
    var subMesh = layer.GetSubMesh(Bottom);
    var count = subMesh.verts.Count;

    subMesh.verts.AddRange(SubstructureBottomVerts);
    subMesh.uvs.AddRange(SubstructureBottomUVs);
    subMesh.colors.AddRange(DefaultColors);
    for (var i = 0; i < 6; i++)
    {
      subMesh.tris.Add(count + index[i]);
    }

    FinalizeVerts(subMesh, count, center, flipped, extraRotation);
    var altitude = AltitudeLayer.TerrainScatter.AltitudeFor(0.1f);
    subMesh.verts[count] = subMesh.verts[count].WithY(altitude);
    subMesh.verts[count + 1] = subMesh.verts[count + 1].WithY(altitude);
    subMesh.verts[count + 2] = subMesh.verts[count + 2].WithY(altitude);
    subMesh.verts[count + 3] = subMesh.verts[count + 3].WithY(altitude);
  }
