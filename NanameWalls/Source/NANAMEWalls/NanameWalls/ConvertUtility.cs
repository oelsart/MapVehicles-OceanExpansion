...
public static class ThingConvertUtility
{
  public static void ConvertThingWithDef(Thing thing, ThingDef def)
  {
    ...
    if (thing.TryGetComp<CompPowerBattery>(out var compBattery1) &&
        newThing.TryGetComp<CompPowerBattery>(out var compBattery2))
      compBattery2.SetStoredEnergyPct(compBattery1.StoredEnergyPct);

    if (thing.TryGetComp<CompRefuelable>(out var compRefuelable1) &&
        newThing.TryGetComp<CompRefuelable>(out var compRefuelable2))
